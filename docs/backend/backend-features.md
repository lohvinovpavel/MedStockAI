...
| F2 | Supplier + catalog | Suppliers, lead time, reliability, per-SKU unit cost — what drives flow 14's live estimate | `warehouse` | `GET /api/warehouse/suppliers` · `GET /api/warehouse/suppliers/{id}/catalog` · `POST /api/warehouse/quote` | Order pricing reads it; `ingest-pricing` can seed it from NADAC | New `supplier`, `supplier_catalog` (supplier_id, ndc, unit_cost, uq pair) | 14 | ✅ wave 4 — `/orders` still mock until F3 |
