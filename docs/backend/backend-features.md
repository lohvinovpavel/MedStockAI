...
| B2 | Facility-scoped stock read | Resolve RxCUI → NDCs → on-hand rows for one facility | `inventory` | `GET /api/inventory/stock?rxcui=&facility_id=` · `GET /api/inventory/items` | Calls the shared `rxnorm.py` client | R `stock_snapshot` (uq `hospital_id, ndc, facility_id, location_id` since B1), left join `formulary_item` | 3, 4, 7, 14 | ⚠️ spec'd; service serves only `/healthz` |
