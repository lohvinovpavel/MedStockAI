...
| E2 | Days-of-supply / at-risk | The product's core metric; depletion date per SKU | `prediction` | `GET /api/prediction/at-risk` · `GET /api/prediction/days-of-supply` | Joins stock against shortages — the join that justified one shared database | R `stock_snapshot` ⋈ `forecast_point` ⋈ `shortage_event` | 4, 9, 16 | ✅ on `/forecasts`; B3 exposure attaches trailing-mean days; shortage KPIs still mock |
